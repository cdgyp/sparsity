...
class LoRAMultiheadAttention(MultiheadAttention, lora.LoRALayer):
    def __init__(self, 
            embed_dim, num_heads, dropout=0, bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None, batch_first=False, device=None, dtype=None,
            r=0, lora_alpha=1, lora_dropout=0., merge_weights=True
        ) -> None:
        MultiheadAttention.__init__(self, embed_dim, num_heads, dropout, bias, add_bias_kv, add_zero_attn, kdim, vdim, batch_first, device, dtype)
        lora.LoRALayer.__init__(self, r=r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, merge_weights=merge_weights)
        # Actual trainable parameters
        if r == 0 or lora_dropout != 0.0:
            raise NotImplemented
        if r > 0:
            def make_lora(weight: torch.Tensor):
                lora_linear = lora.Linear(weight.shape[-1], weight.shape[-2], r, lora_alpha=lora_alpha, lora_dropout=lora_dropout, merge_weights=merge_weights, bias=False, dtype=weight.dtype)
            ...
            if self._qkv_same_embed_dim:
                self.lora_in_proj = lora.MergedLinear(self.in_proj_weight.shape[-1], self.in_proj_weight.shape[-2], r, lora_alpha, lora_dropout, [True, True, True], merge_weights=merge_weights, bias=False)
                self.lora_in_proj.reset_parameters()
                ...
                self.lora_v = make_lora(self.v_proj_weight)
    ...
    Outputs:
        ...
        if not why_not_fast_path:
            tensor_args = (
                ...
                key,
                value,
                self.in_proj_weight + self.lora_in_proj.weight,
                self.in_proj_bias,
                self.out_proj.weight + self.lora_out_proj.weight,
                self.out_proj.bias,
            )
            ...
            if not why_not_fast_path:
                ...
                assert self.lora_in_proj.merged and self.lora_out_proj.merged # so directly adding self.lora_in_proj.weight to the non-trainable weight will suffice
    ...
    def _in_projection_packed(self,
        ...
        if k is v:
            if q is k:
                # self-attention
                proj = linear(q, w, b) + self.lora_in_proj(q)
                ...
                q_proj = linear(q, w_q, b_q) + self.lora_in_proj()
    ...
    def multi_head_attention_forward(self,
        ...
        if need_weights:
            ...
            assert not out_proj_weight.requires_grad
            attn_output = linear(attn_output, out_proj_weight, out_proj_bias) + self.lora_out_proj(attn_output)
...
class LoRAfy:
    ...
    def replace_linears(self, model: torch.nn.Module, path='model'):
        for name, module in model.named_children():
            ...
            if isinstance(module, torch.nn.Linear) and not isinstance(module, lora.LoRALayer):
            ...
            elif isinstance(module, MultiheadAttention) and not isinstance(module, lora.LoRALayer):
                dtype = None
                ...
                self.linears.append(p + ': ' + str(module.__class__))
            elif isinstance(module, torch.nn.Conv2d) and not isinstance(module, lora.LoRALayer) and False:
                assert len(module.kernel_size) == 2 and module.kernel_size[0] == module.kernel_size[1]
